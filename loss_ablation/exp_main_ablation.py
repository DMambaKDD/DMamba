...
import models.DMamba_TrendMamba as DMamba_trend
import models.DMamba_MLP as DMamba_mlp
import models.DMamba_AllMamba as DMamba_all_mamba
import models.DMamba_MixedMamba as DMamba_mixed_mamba
import models.DMamba_Hybrid as DMamba_hybrid
from loss_ablation.models import iTransformer, smamba, TimesNet
...
class Exp_Main(Exp_Basic):
    ...
    def _build_model(self):
        # Extend model dict to include external models for ablation
        model_dict = {
            'DMamba': DMamba,
            'DMamba_trend': DMamba_trend,
            'DMamba_mlp': DMamba_mlp,
            'DMamba_all_mamba': DMamba_all_mamba,
            'DMamba_mixed_mamba': DMamba_mixed_mamba,
        ...
        with torch.no_grad():
            for i, (batch_x, batch_y, batch_x_mark, batch_y_mark) in enumerate(vali_loader):
                ...
                if not is_test:
                    ...
                    if self.args.model == 'DMamba_mse':
                        # For DMamba_mse, we don't use ratio weighting
                        pred = outputs
                        true = batch_y
                    else:
                        # Use same logic as DMamba for all models in this ablation
            ...
            for i, (batch_x, batch_y, batch_x_mark, batch_y_mark) in enumerate(train_loader):
                ...
                if self.args.use_amp:
                    ...
                    if self.args.model == 'DMamba_mse':
                        ...
                        # Arctangent loss with weight decay
                        # Using alpha=0.3 implicitly by not changing the ratio calculation?
                        # Wait, user asked for "Arctangent Loss alpha=0.3". 
                        # The code for ratio is: -1 * math.atan(i+1) + math.pi/4 + 1
                        # This formula doesn't have an explicit 'alpha' parameter. 
                        # It seems the user might be referring to the EMA alpha=0.3? 
                        # Or maybe the arctan formula itself is considered fixed.
                        # I will stick to the existing Arctan implementation in DMamba.
